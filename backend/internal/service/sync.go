...
func (s *SyncService) SyncConnection(ctx context.Context, conn *model.ProviderConnection) error {
	...
	for _, txn := range txns {
		// Check if transaction already exists to avoid overwriting user edits
		existing, _ := s.txnRepo.GetByPK(ctx, txn.PK)
		if existing != nil {
			// Preserve user edits: skip if confirmed or has a user-assigned category
			if existing.IsConfirmed || existing.CategoryID != nil {
				count++
				continue
			}
